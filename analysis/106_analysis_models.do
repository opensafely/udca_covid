    ...
    * Open file to write results
    ...
    if r(N) > 10 {
        ...
        * Cox model - age and sex adjusted
        ...
        estat phtest, detail
        * Cox model - fully adjusted
        stcox udca age_tv male any_high_risk_condition i.eth_bin i.imd ib2.bmi_cat i.smoking severe_disease i.vacc_count_tv liver_trans, strata(stp) vce(robust)
        estimates save "./output/tempdata/adj_model_`outcome'", replace 
        eststo model9
        ...
        * Cox model - fully adjusted with binary ethnicity variable 
        stcox udca age_tv male any_high_risk_condition i.ethnicity i.imd ib2.bmi_cat i.smoking severe_disease i.vacc_count_tv liver_trans, strata(stp) vce(robust)
        * flag single row for each person
        bys patient_id (start): gen number = _n==_N 
        tab number 
        * Count number unexposed at any time 
